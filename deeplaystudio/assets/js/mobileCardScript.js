...
function mobile_teams_handleMouseMove(event){
    curMousePosition2 = event.clientX ;
    let delta2 = lastMousePosition2 - curMousePosition2;
    deltaMouse2 = curMousePosition2 - lastMousePosition2 ;
    lastMousePosition2 = curMousePosition2 ;
    angleOffset2 += delta2 * mouseSensitivity2 ;
    lastMouseMoveTime = Date.now();
    setAngleOffset2(angleOffset2);
}
...
function mobile_teams_handleTouchMove(event){
    ...
    deltaMouse2 = curMousePosition2 - lastMousePosition2 ;
    lastMousePosition2 = curMousePosition2 ;
    angleOffset2 += delta2 * touchSensitivity2 ;
    setAngleOffset2(angleOffset2);
}
...
function mobile_teams_throttle(fn2, wait2) {
    var time2 = Date.now();
    return function(event) {
      if ((time2 + wait2 - Date.now()) < 0) {
        fn2(event);
        time2 = Date.now();
      }
    }
...
function mobile_teams_distribute(cards2) {
    if(cards2.length == 0) 
        return ;
    let angle2 = Math.PI*2/cards2.length ;
    unitAngle2 = 360 / cards2.length ;
    let radius2 = cards2[0].offsetWidth/(2*Math.tan(angle2/2)) + 16;
    slider2.style.transformOrigin = `center center ${-radius2}px` ;
    cards2.forEach((card,index)=> {
        let tiltAngle = index*angle2 ;
        let deltaZ = radius2 * (1 - Math.cos(tiltAngle));
        let deltaY = radius2 * Math.sin(tiltAngle) ;
...
function setAngleOffset2(newOffset){
...
window.addEventListener("load",() => {
    word2 = document.querySelector(".name");
    orignal2 = `itsGoodBits`;
    l2=orignal2.length;
    j2=c2=p2=0;
    clock222 = setInterval(mobile_teams_shuffle,30);
});

function mobile_teams_shuffle(){
    if(p2-->0) return;
    text="";
    for(var k2=0;k2<j2;k2++) text2 += orignal2[k2];
    for(var k2=j2;k2<j2+4 && k2<l2;k2++){
        text2 += String.fromCharCode(
            (Math.random()>0.5)?
